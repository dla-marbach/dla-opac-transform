...
OAI_DC_XSD_LOCAL = os.path.join(os.path.dirname(__file__), "oai_dc.xsd")
...
def validate_dc_xml(xml_content):
    with tempfile.NamedTemporaryFile("w+", suffix=".xml", delete=False) as tmp:
        ...
        tmp_path = tmp.name
    try:
        # Setze das Arbeitsverzeichnis auf das Scriptverzeichnis, damit relative XSD-Imports funktionieren
...
def main(csv_path):
    ...
    with open(csv_path, newline='', encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not dc_xml.strip():
                print(f"{rec_id}: Kein oai_dc-XML vorhanden.")
                continue
